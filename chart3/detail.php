<?php
	...
	$get_sumber = isset($_POST['sumber']) ? $_POST['sumber'] : false;
	...
	if($get_download && $get_download == 'yes'){
		...
		$get_sumber = isset($_GET['sumber']) ? $_GET['sumber'] : false;
	...
	foreach ($data_wika as $value) {
		...
		if($month <= $get_bulan){
			...
			if($get_company == $company){
				if($sumber == $get_sumber){
					if($get_jenis == 'Terendah' && $status == 'Terendah'){
						$dataRow = array();	
